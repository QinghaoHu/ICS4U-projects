...
public class VehicleWorld extends World
{
    ...
    private int actsBetweenRain, rainDuration;
    ...
    {    
        ...
        setPaintOrder (Effect.class, Pedestrian.class, Vehicle.class); 

        // set up background -- If you change this, make 100% sure
        // that your chosen image is the same size as the World
        background = new GreenfootImage ("background02.png");
    ...
    private void spawn () {
        ...
        if (Greenfoot.getRandomNumber (120) == 0){
            Rock rock = new Rock();
            int xSpawnLocation = Greenfoot.getRandomNumber(getWidth() - 100) + 100; // keep away from the left edge
            int ySpawnLocation = Greenfoot.getRandomNumber(getHeight() - 300) + 300; // keep away from the left edge
            addObject(new Rock(), xSpawnLocation, ySpawnLocation);
        }

        // Chance to spawn a Pedestrian
        if (Greenfoot.getRandomNumber (30) == 0 && getObjects(Pedestrian.class).size() < 8){
            int xSpawnLocation = Greenfoot.getRandomNumber(getWidth() - 100) + 100; // keep away from the left edge
            boolean spawnAtTop = Greenfoot.getRandomNumber(2) == 0;

            int type = Greenfoot.getRandomNumber(2);
            Pedestrian ped;
            if (type == 0){ // Rock Monster
                if (spawnAtTop){
                    addObject (new RockMonster (1), xSpawnLocation, TOP_SPAWN);
                } else {
                    addObject (new RockMonster (-1), xSpawnLocation, BOTTOM_SPAWN);
                }
            } else if (type == 1){
                if (spawnAtTop){
                    addObject (new FireElemental (1), xSpawnLocation, TOP_SPAWN);
                } else {
                    addObject (new FireElemental (-1), xSpawnLocation, BOTTOM_SPAWN);
                }
            }
